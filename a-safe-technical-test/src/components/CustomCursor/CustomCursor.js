...
export default function CustomCursor({ size = 50, smooth = 0.2 }) {
  ...
  useEffect(() => {
    const onMove = (e) => {
      ...
      setIsLeft(e.pageX < window.innerWidth / 2);
      if (firstMove.current) {
        firstMove.current = false;
        current.current.x = e.pageX;
        current.current.y = e.pageY;
        setRenderPos({ x: e.pageX, y: e.pageY });
      }
      setVisible(true);
    };
